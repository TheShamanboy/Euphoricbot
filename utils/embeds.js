...
• ✨ **Misc** - Epic special roles (0.15% chance)`)
      .setColor('#7289da')
      .setImage('https://images.steamusercontent.com/ugc/942824057165995314/3BD7BBED6FBA65BF84598AB1B4010A28C2E5A954/?imw=5000&imh=5000&ima=fit&impolicy=Letterbox&imcolor=%23000000&letterbox=false')
      .setTimestamp();
  }
...
async function handleSpinButton(userData, spinType) {
  ...
  return {
    embed: new EmbedsClass().createSpinResultsEmbed(results, userData.currency),
...
async function onButtonInteraction(interaction) {
  ...
  if (interaction.customId === 'profile') {
    ...
    const profileEmbed = new EmbedsClass().createProfileEmbed(userData, canClaimDaily);
    await interaction.reply({ embeds: [profileEmbed], ephemeral: true });
  } else if (interaction.customId === 'spin') {
    const spinEmbed = new EmbedsClass().createSpinEmbed(userData);
    const spinButtons = new EmbedsClass().createSpinButtons(userData);
    ...
    const shopEmbed = new EmbedsClass().createShopEmbed(userData);
    const shopMenu = new EmbedsClass().createShopMenu();
    await interaction.reply({ embeds: [shopEmbed], components: [shopMenu], ephemeral: true });
  } else if (interaction.customId === 'inventory') {
    const inventoryEmbed = new EmbedsClass().createInventoryEmbed(userData);
    const inventoryButtons = new EmbedsClass().createInventoryButtons(userData);
...
export const Embeds = new EmbedsClass();
export { handleSpinButton, onButtonInteraction };
